...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C2f_Strip', 'C2f_StripCGLU', 'MultiScalePCA', 'FSA', 'MultiScalePCA_Down', 'C2f_KAT', 'C2f_Faster_KAN', 'C2f_DCMB', 'C2f_DCMB_KAN', 'C2f_GlobalFilter', 'C2f_DynamicFilter', 'HAFB','HAFB_Dynamic',
...
class HAFB(nn.Module):
    ...
    def __init__(self, inc, ouc, group=False):
       ...
       # self.lgb2_global = LocalGlobalAttention(hidc, 4)
        self.W_x2_adjust = Conv(hidc, ouc, 1, act=False) #  单路通道对齐
    ...
    def forward(self, inputs):
       ...
       # x2 = torch.cat([self.lgb2_local(W_x2), self.lgb2_global(W_x2)], dim=1)
        x2_adj = self.W_x2_adjust(W_x2)  # [B, ouc, H, W] #  单路通道对齐
...
######################################## Hierarchical Attention Fusion Block start1 ########################################

class HAFB_Dynamic(nn.Module):
    def __init__(self, inc, ouc, group=False):
        super().__init__()
        ch_1, ch_2 = inc
        hidc = ouc // 2
      ...
      #  self.lgb2_local = LocalGlobalAttention(hidc, 2)
      #  self.lgb2_global = LocalGlobalAttention(hidc, 4)
        self.W_x2_adjust = Conv(hidc, ouc, 1, act=False) #
        # 通道变换层
        self.W_x1 = Conv(ch_1, hidc, 1, act=False)
        self.W_x2 = Conv(ch_2, hidc, 1, act=False)
        self.W = Conv(hidc, ouc, 3, g=4)
        ...
        # 门控生成器 - 关键部分
        # 输入: W_x1 + W_x2 的拼接 [B, hidc*2, H, W]
        # 输出: 每个空间位置的权重 [B, 1, H, W]
        self.gate_conv = nn.Sequential(
            Conv(hidc * 2, hidc // 2, 1),  # 降维
            Conv(hidc // 2, 1, 1, act=nn.Sigmoid())  # 生成0-1之间的权重
        )
    ...
    def forward(self, inputs):
        ...
        # 步骤1: 基础特征提取
        W_x1 = self.W_x1(x1)  # [B, hidc, H, W]
        W_x2 = self.W_x2(x2)  # [B, hidc, H, W]
        bp = self.W(W_x1 + W_x2)  # 基础路径
        
        # 步骤2: 第一路注意力处理
        x1_local = self.lgb1_local(W_x1)
        x1_global = self.lgb1_global(W_x1)
        x1_att = torch.cat([x1_local, x1_global], dim=1)  # [B, ouc, H, W]
        
        # 步骤3: 第二路的两种处理方式
        # 3.1 注意力路径
      #  x2_local_att = self.lgb2_local(W_x2)
      #  x2_global_att = self.lgb2_global(W_x2)
      #  x2_att = torch.cat([x2_local_att, x2_global_att], dim=1)  # [B, ouc, H, W]
        
        # 3.2 非注意力路径
        x1_adj = self.W_x2_adjust(W_x1)  # [B, ouc, H, W]
        x2_adj = self.W_x2_adjust(W_x2)  # [B, ouc, H, W]
        
        # 步骤4: 门控机制生成
        # 4.1 拼接两路特征作为门控输入
        gate_input = torch.cat([W_x1, W_x2], dim=1)  # [B, hidc*2, H, W]
        
        # 4.2 生成门控权重图
        gate = self.gate_conv(gate_input)  # [B, 1, H, W], 值在0-1之间
        
        # 步骤5: 门控融合
        # 公式: x2_fused = gate * x2_att + (1 - gate) * x2_adj
        x1_fused = gate * x1_att + (1 - gate) * x1_adj  # [B, ouc, H, W]
        
        # 步骤6: 三路特征融合
        combined = torch.cat([x1_fused, x2_adj, bp], 1)  # [B, ouc*3, H, W]
        
        # 步骤7: 后续处理
        squeezed = self.conv_squeeze(combined)
        rep = self.rep_conv(squeezed)
        output = self.conv_final(rep)
